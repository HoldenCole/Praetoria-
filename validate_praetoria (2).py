...
# §4b: these counters are engine-written projections of the player's house — READ only, never adjustCounter
READONLY_COUNTERS={"title_rank","house_legitimacy","title_instability"}

# === §8 LOCKED CONTROLLED VOCABULARIES (canonical bible — see PRAETORIA_VOCAB.md) ===
NATURE={"Ambitious","Cruel","Honorable","Just","Loyal","Proud","Ruthless","Vengeful"}
...
AMBITIONS={"amass_a_fortune","earn_a_command","master_the_senate","outshine_all_rivals","restore_house_fortunes","seize_the_throne","win_a_great_love"}
        ...
        for ch in e.get("choices",[]):
            ...
            for ef in ch.get("effects",[]):
                ...
                if ef["type"]=="adjustCounter" and ef.get("key") in READONLY_COUNTERS:
                    errors.append(f"[{eid}/{cid}] adjustCounter on READ-ONLY counter '{ef.get('key')}' (engine-written; use setTitle/adjustLegitimacy/grantClaim)")
